"""Tests for distribute_qlateral_to_links — lateral flow distribution to discretized links."""
import pytest
import pandas as pd
import numpy as np

from troute.NHF import distribute_qlateral_to_links


def _make_links(records):
    """Helper: build links_df from list of dicts with link_id, fp_id, div_id."""
...
class TestDistributeQlateralToLinks:
    """Tests for distribute_qlateral_to_links function."""
    ...
    def single_divide_with_vfps(self):
        ...
        specific links. Remainder (50%) distributed equally.
        ...
        links_df = _make_links([
            {'link_id': 1001, 'fp_id': 10, 'div_id': 1},
            {'link_id': 1002, 'fp_id': 10, 'div_id': 1},
            {'link_id': 1003, 'fp_id': 10, 'div_id': 1},
        ])
        ...
        return div_lateralflows_df, vfp_dataframe, links_df, nodes_df, reference_flowpaths

    def test_vfp_qlat_goes_to_target_link(self, single_divide_with_vfps):
        """VFP qlat should be assigned to the link downstream of its terminal nexus."""
        div_qlats, vfp_df, links_df, nodes_df, ref_fps = single_divide_with_vfps

        routing_qlats, _ = distribute_qlateral_to_links(
            div_qlats, vfp_df, links_df, nodes_df, ref_fps
        )

        # vfp_30 (20%) → node_901 → link_1001:  100 * 0.2 = 20
        # vfp_31 (30%) → node_902 → link_1002:  100 * 0.3 = 30
        # Remainder 50% / 3 links = 16.667 per link
        remainder_per_link = 100.0 * 0.5 / 3

        assert routing_qlats.loc[1001, '202001010000'] == pytest.approx(20.0 + remainder_per_link)
        assert routing_qlats.loc[1002, '202001010000'] == pytest.approx(30.0 + remainder_per_link)
    ...
    def test_mass_balance(self, single_divide_with_vfps):
        """Total qlat across all links must equal total divide qlat."""
        div_qlats, vfp_df, links_df, nodes_df, ref_fps = single_divide_with_vfps
    ...
    def test_multiple_timestamps(self, single_divide_with_vfps):
        ...
        # t=1: div_qlat=110
        remainder_per_link = 110.0 * 0.5 / 3
        assert routing_qlats.loc[1001, '202001010100'] == pytest.approx(110 * 0.2 + remainder_per_link)
        assert routing_qlats.loc[1002, '202001010100'] == pytest.approx(110 * 0.3 + remainder_per_link)
    ...
    def test_flow_scaling_df_preserves_vfp_values(self, single_divide_with_vfps):
        ...
        _, flow_scaling_df = distribute_qlateral_to_links(
            div_qlats, vfp_df, links_df, nodes_df, ref_fps
        )
    ...
    def test_divide_without_vfps(self):
        """A divide with no VFPs distributes all qlat equally among its links."""
        ...
        links_df = _make_links([
            {'link_id': 2001, 'fp_id': 20, 'div_id': 1},
            {'link_id': 2002, 'fp_id': 20, 'div_id': 1},
        ])
        nodes_df = _make_nodes([])
        ref_fps = _make_ref_fps([{'fp_id': 20, 'virtual_fp_id': np.nan, 'div_id': 1}])

        routing_qlats, flow_scaling_df = distribute_qlateral_to_links(
            div_qlats, vfp_df, links_df, nodes_df, ref_fps
        )
    ...
    def test_multiple_divides(self):
        ...
        links_df = _make_links([
            # fp_id=10, div_id=1: 2 links
            {'link_id': 1001, 'fp_id': 10, 'div_id': 1},
            {'link_id': 1002, 'fp_id': 10, 'div_id': 1},
            # fp_id=20, div_id=2: 2 links
            {'link_id': 2001, 'fp_id': 20, 'div_id': 2},
            {'link_id': 2002, 'fp_id': 20, 'div_id': 2},
        ])
        nodes_df = _make_nodes([
            {'node_id': 901, 'dn_link_id': 1001, 'fp_id': 10, 'is_terminal_nexus': True},
        ])
        ...
        # div_1: VFP 40% → link_1001, remainder 60% / 2 = 30 each
        assert routing_qlats.loc[1001, '202001010000'] == pytest.approx(40.0 + 30.0)
        assert routing_qlats.loc[1002, '202001010000'] == pytest.approx(30.0)

        # div_2: no VFPs, 200 / 2 = 100 each
        ...
        # mass balance
        total = routing_qlats['202001010000'].sum()
        assert total == pytest.approx(300.0)

    def test_multiple_vfps_to_same_terminal_nexus(self):
        """Multiple VFPs draining to the same terminal nexus accumulate on one link."""
        div_qlats = pd.DataFrame(
            {'202001010000': [100.0]},
            index=[1],
        )
        ...
        # 3 VFPs all draining to terminal nexus 901
        vfp_df = pd.DataFrame(
            {
                'div_id': [1, 1, 1],
        ...
        # All 3 VFPs (60% total) → link_1001
        # Remainder 40% / 2 links = 20 each
        assert routing_qlats.loc[1001, '202001010000'] == pytest.approx(60.0 + 20.0)
    ...
    def test_vfp_without_mapped_terminal_nexus(self):
        """VFP whose dn_virtual_nex_id is not a terminal nexus node distributes to all links."""
        div_qlats = pd.DataFrame(
            {'202001010000': [100.0]},
            index=[1],
        )
        ...
        # VFP drains to nex_999 which is NOT in nodes_df as a terminal nexus
        vfp_df = pd.DataFrame(
            {
                'div_id': [1],
                'percentage_area_contribution': [0.4],
        ...
        # VFP 40% has no mapped terminal nexus → spread across 2 links: 20 each
        # Remainder 60% / 2 = 30 each
        # Total per link: 50
    ...
    def test_single_link_flowpath(self):
        """Flowpath with a single link receives all qlat."""
        div_qlats = pd.DataFrame(
            {'202001010000': [80.0]},
            index=[1],
        )
        ...
        # 901 not in nodes as terminal nexus → VFP distributed to all links (1 link)
        # VFP: 80 * 0.5 = 40 → link_1001
        # Remainder: 80 * 0.5 = 40 → link_1001
    ...
    def test_total_flow_conserved_multiple_divides_timestamps(self):
        ...
        links_df = _make_links([
            ...
            {'link_id': 1002, 'fp_id': 10, 'div_id': 1},
            {'link_id': 2001, 'fp_id': 20, 'div_id': 2},
            {'link_id': 3001, 'fp_id': 30, 'div_id': 3},
            {'link_id': 3002, 'fp_id': 30, 'div_id': 3},
        ])
        nodes_df = _make_nodes([
            {'node_id': 901, 'dn_link_id': 1001, 'fp_id': 10, 'is_terminal_nexus': True},
        ...
        for col in div_qlats.columns:
            ...
            total_out = routing_qlats[col].sum()
