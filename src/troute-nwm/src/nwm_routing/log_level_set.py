...
def use_ewts_logger():
    import ewts
    from ewts.logger import EwtsLogger, EwtsHandler

    ewts_logger = EwtsLogger(ewts.T_ROUTE_ID)

    logger = logging.getLogger("troute")
    logger.handlers.clear() 
    logger.propagate = False    
    logger.setLevel(ewts_logger._min_level)
    
    logger.addHandler(EwtsHandler(ewts_logger))
