...
def read_geopkg(file_path, compute_parameters, waterbody_parameters, cpu_pool):
    ...
    if cpu_pool > 1:
        ...
        flowpaths = pd.merge(
            table_dict.get("flowpaths"), table_dict.get("flowpath-attributes"), on="id", suffixes=("", "_y")
        )
    ...
    if "hydrolocations" in layers:
        hydro = gpd.read_file(file_path, layer="hydrolocations")
...
class HYFeaturesNetwork(AbstractNetwork):
    ...
    def preprocess_network(self, flowpaths, nexus):
        ...
        self._connections = extract_connections(self.dataframe, "downstream", terminal_codes=self.terminal_codes)
    ...
    def preprocess_waterbodies(self, lakes, nexus):
        ...
        if not lakes.empty:
            ...
            if not great_lakes_df.empty:
                ...
                self._waterbody_df = pd.concat([self.waterbody_dataframe, gl_wbody_df])
            else:
                gl_dict = {}
